use crate::types::{Ast, BinOp, Env, Storage, Type};

use nom::branch::alt;
use nom::bytes::complete::{tag, take_until};
use nom::character::complete::{
    alpha1, alphanumeric0, char, digit1, multispace0, multispace1, one_of,
};
use nom::multi::many0;
...
impl Ast {
    ...
    fn literal(input: &str) -> IResult<&str, Ast> {
        alt((Ast::lit_u32, Ast::lit_bool, Ast::lit_var))(input)
    }
    ...
    fn ty(input: &str) -> IResult<&str, Type> {
        alt((Ast::ty_u32, Ast::ty_bool, Ast::ty_void))(input)
    }
    ...
    fn block(input: &str) -> IResult<&str, Ast> {
        ...
        Ok((
            ...
            Ast::Block {
                ty: Type::Void,
                nodes: nodes.into_iter().map(|(ast, _, _, _)| ast).collect(),
            },
        ))
    }

    fn expr(input: &str) -> IResult<&str, Ast> {
        alt((Ast::bin_op, Ast::literal, Ast::block))(input)
    }
        ...
        match self {
            ...
            Ast::Literal {
                ...
                Type::Var => *env
                    .types
                    .get(content)
                    .unwrap_or_else(|| panic!("Variable {content:?}'s type undefined")),
                ty => *ty,
            },
            Ast::BinOp {
                bin_op: _,
    ...
    fn comment(input: &str) -> IResult<&str, &str> {
        let (input, (_, body, _)) = tuple((tag("//"), take_until("\n"), tag("\n")))(input)?;
    ...
    pub fn parse(mut input: &str) -> IResult<&str, Vec<Ast>> {
        ...
        while !input.is_empty() {
            ...
            let (output, (ast, _, _sq)) =
                tuple((alt((Ast::assignment, Ast::expr)), multispace0, char(';')))(input)?;
            ...
            Ast::Literal {
                ...
                output += &match ty {
                        ...
                        .unwrap_or_else(|| panic!("Variable {content:?}'s type undefined"))
                    {
                        ...
                        Type::U32 => format!(
                            "mov eax, dword[ebp-{}]\n",
                            env.stack.get(content).unwrap_or_else(|| panic!(
                                "Variable {content:?}'s stack location undefined"
                            ))
                        ),
                        Type::Bool => format!(
                            "movsx eax, byte[ebp-{}]\n",
